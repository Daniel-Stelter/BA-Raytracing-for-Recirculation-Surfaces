...
namespace RS
{
  ...
  public:
    ...
    constexpr mat(row_ts const (&...rows)[C]) : m_data{}
    {
      ...
      auto insert_row = [r = 0UL, this](const auto &row_data) mutable {
        size_t c = 0;
        for (auto v : row_data)
          at(r, c++) = static_cast<real_t>(v);
        ++r;
      };

      // apply insert_row to each row
      utils::for_each(insert_row, rows...);
    }

  private:
    /// constructor that sets all components to c
    constexpr mat(real_t c) : m_data{utils::make_array<real_t, num_components>(c)} {}
  ...
  public:
    ...
    {
      auto m = zeros();
      for (size_t i = 0; i < std::min(num_rows, num_columns); ++i)
        m(i, i) = 1;
      return m;
    }
    //============================================================================
