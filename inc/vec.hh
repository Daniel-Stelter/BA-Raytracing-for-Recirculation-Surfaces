...
namespace RS
{
  ...
  private:
    ...
    explicit constexpr vec(real_t c) : m_data{utils::make_array<real_t, size>(c)} {}
  ...
  //------------------------------------------------------------------------------
  /// normal of the surface spanned by the three vectors.
  /// zero if they are colinear
  // auto surface_normal(const vec3 &v1, const vec3 &v2, const vec3 &v3)
  // {
  ...
  //   if (std::abs(d1.length()) <= Globals::ZERO || std::abs(d2.length()) <= Globals::ZERO)
  //     return vec3{0, 0, 0};
  //   d1 = normalize(d1);
  //   d2 = normalize(d2);
  //   bool b1 = false, b2 = false;
  //   for (size_t i = 0; i < 3; ++i) // check if values are the same (or flipped)
  //   {
  //     if (std::abs(d1.at(i) - d2.at(i)) > Globals::ZERO)
  //       b1 = true;
  //     if (std::abs(d1.at(i) + d2.at(i)) > Globals::ZERO)
  //       b2 = true;
  //   }
